...
`PROMPT.md` is the source of truth for scope, roles, and design tokens — read it before feature work. Of its four setup tasks, **1–4 are done**: scaffold, design system, DB schema + mock data, and the student dashboard / curriculum screens. On top of that, the four admin screens (roster, grading, curriculum CMS, announcements) exist as **read-only base UI**.
...
- **Every write path.** The admin screens read from `lib/mock` and their forms are appearance-only: inputs are uncontrolled, all buttons are `type="button"`, and reorder / 編集 / 配信 do nothing. `components/admin/mock-form.tsx` holds the shared field, select, and "not wired yet" notice. Each of those forms is where a Server Action goes.
- **Admin chat** — PROMPT.md also lists per-student chat for the admin side; the mock threads and `getMessageThreads()` / `getUnreadCount()` helpers exist, but the only surface today is an unread count on the roster.
...
| `components/admin/` | Admin-view pieces (its own header nav, page header, stat tiles, status badges, lesson editor row, mock form parts). Reuses `SubmissionStatusBadge` and `LESSON_TYPE_META` from `components/student/` rather than duplicating them — those two modules are presentational and role-neutral. |
